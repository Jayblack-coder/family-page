...
const NwankwoGenOneProfiles = () => {
  ...
  const isFirstGeneration = (value) => {
    const normalized = String(value || "").trim().toLowerCase();
    return normalized === "1st" || normalized === "1" || normalized === "first";
  };
  ...
  useEffect(() => {
    ...
    const newFilteredData = originalData.filter((item) => {
      ...
      const hasGeneration = isFirstGeneration(item.generation);
      ...
      {new Date(member.dateOfDeath).toLocaleDateString("en-GB")}
              ...
              <Box sx={{ width: "100%", textAlign: "center", py: { xs: 3, sm: 4, md: 5 } }}>
                <Typography variant="body1" color="text.secondary" sx={{ fontSize: { xs: "0.9rem", sm: "1rem", md: "1.1rem" } }}>
                  No members found for Nwankwo Generation 1
                </Typography>
              </Box>
            )}
          </Grid>
